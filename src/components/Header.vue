<template>
  <header class="navbar navbar-expand-xl py-3" :class="[{ 'hero-header': $route.path === '/' }]">
    <section class="container">
      <router-link to="/" class="navbar-brand">
        <img :src="masterStore?.masterData?.logo" height="50px" class="object-fit-cover" alt="ReadyLMS" />
      </router-link>
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <div class="category-dropdown dropdown">
          <button class="btn category-dropdown-btn mt-3 mx-3 px-3" type="button" data-bs-toggle="dropdown"
            aria-expanded="false">
            <img src="@/assets/images/website/category.svg" class="me-2" />
            Category<i class="ri ri-arrow-down-s-line ms-4"></i>
          </button>
          <ul class="category-menu dropdown-menu border-0 theme-shadow p-3">
              ...
              <router-link :to="'/courses?category_id=' + category.id" class="dropdown-item" href="#">
                <div class="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <small>{{ category.course_count }} Courses</small>
                  </div>
                </div>
              </router-link>
            </li>
        ...
        <ul class="navbar-nav ms-4 me-auto mb-2 mb-lg-0">
          <li class="nav-item">
            <router-link to="/" :class="['nav-link', $route.path === '/' ? 'active' : '']">Home</router-link>
          </li>
          <li class="nav-item">
            <router-link to="/courses"
              :class="['nav-link', $route.path === '/courses' ? 'active' : '']">Courses</router-link>
          </li>
          <li class="nav-item">
          ...
          <li class="nav-item">
            <router-link to="/contact-us" :class="['nav-link', $route.path === '/contact-us' ? 'active' : '']"
              class="nav-link" href="#">Contact Us</router-link>
          </li>
        </ul>
        <div v-if="authStore.authToken" class="dropdown me-3">
          <a href="#" class="text-decoration-none text-dark" data-bs-toggle="dropdown" aria-expanded="false">
            <img :src="authStore.userData.profile_picture" class="rounded-circle object-fit-cover" alt="Menu"
        ...
        <div v-else>
          <router-link to="/login" class="btn btn-primary rounded-pill me-3">Login</router-link>
        </div>
      </div>
    </section>
  </header>
