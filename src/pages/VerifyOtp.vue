          ...
          <div class="d-flex h-100">
            <div class="my-auto w-100">
              ...
              <span class="text-muted"
                >We sent an OTP code to your email address - <strong>{{ email }}</strong></span
              >
              ...
              <form @submit.prevent="verifyOtp" class="my-4">
                <div class="otp-fields d-flex mb-4">
                  <input
                    type="number"
                    class="form-control text-center py-3 me-3"
                    maxlength="1"
                ...
                <span v-if="resendTimeLeft !== '00'" class="d-block mb-3"
                  >Resend OTP in <span class="text-primary">00:{{ resendTimeLeft }}</span></span
                >
                <a v-else @click="resendOtp" class="text-primary d-block">Resend OTP</a>
                <button type="submit" class="btn btn-primary w-100 rounded-pill mt-3">Confirm OTP</button>
              </form>
            </div>
          </div>
        </div>
